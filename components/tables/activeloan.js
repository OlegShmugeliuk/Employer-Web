...
export default function ActiveLoan({ benefiUser }) {
    ...
    const [forgiveBonusId, setForgiveBonusId] = React.useState(null);
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <>
                        {params.row?.signedBonus
                            ? `${params.row?.signedBonus?.percentage}`
                            : ''}
                    </>
                );
            },
            ...
            renderCell: (params) => {
                ...
                return (
                    <>
                        <Button
                            onClick={() => {
                                setForgiveBonusId(params.row?.id);
    ...
    const { data: bonuses, refetch: refetchBonuses } = useQuery(
        ['getPlanById', benefiUser.id],
        EmployeeService.getBonusesByEmployerId
    );

    const { mutateAsync } = useMutation(EmployeeService.forgiveEmployee, {
        onSuccess: () => {
            successNotify('Employee was forgiven successfully.  ');
            setForgiveBonusId(null);
            refetchBonuses();
            handleCloseForgiveModal();
        },
    });

    const handleForgive = async () => {
        await mutateAsync(forgiveBonusId);
    };
